...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">@csrf
                @method('PUT')
                ...
                <div class="mb-4"><label>{{ __('Content') }}</label>
                    <textarea name="content" rows="12" class="w-full px-4 py-2 border rounded-lg">{{ old('content', $post->content) }}</textarea>
                </div>
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div><label>{{ __('Author') }}</label><input type="text" name="author"
                            value="{{ old('author', $post->author) }}" class="w-full px-4 py-2 border rounded-lg"></div>
                ...
                <div class="mb-4">
                    @if ($post->featured_image)
                        <img src="{{ asset('storage/' . $post->featured_image) }}" class="h-24 mb-2 rounded">
                    @endif
                    <label>
                        {{ __('Featured Image') }}</label><input type="file" name="featured_image" accept="image/*"
                ...
                <div class="mb-6 flex space-x-6"><label class="flex items-center"><input type="checkbox" name="is_featured"
                            value="1" {{ $post->is_featured ? 'checked' : '' }} class="mr-2">Featured</label><label
                        class="flex items-center"><input type="checkbox" name="is_active" value="1"
                            {{ $post->is_active ? 'checked' : '' }} class="mr-2">Active</label></div>
