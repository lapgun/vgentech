...
@section('title', __('Edit Job Posting'))
@section('page-title', __('Edit Job Posting'))
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.recruitments.update', $recruitment) }}" method="POST">@csrf @method('PUT')
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div class="col-span-2"><label class="block text-gray-700 font-medium mb-2">{{ __('Job Title') }}
                            *</label><input type="text" name="title" value="{{ old('title', $recruitment->title) }}"
                            class="w-full px-4 py-2 border rounded-lg" required></div>
                    <div class="col-span-2"><label>{{ __('Description') }}</label>
                        <textarea name="description" rows="4" class="w-full px-4 py-2 border rounded-lg">{{ old('description', $recruitment->description) }}</textarea>
                    </div>
                    <div class="col-span-2"><label>{{ __('Requirements') }}</label>
                        <textarea name="requirements" rows="4" class="w-full px-4 py-2 border rounded-lg">{{ old('requirements', $recruitment->requirements) }}</textarea>
                    </div>
                    <div class="col-span-2"><label>{{ __('Benefits') }}</label>
                        <textarea name="benefits" rows="4" class="w-full px-4 py-2 border rounded-lg">{{ old('benefits', $recruitment->benefits) }}</textarea>
                    </div>
                    ...
                    <div class="col-span-2"><label>{{ __('Contact Email') }}</label><input type="email"
                            name="contact_email" value="{{ old('contact_email', $recruitment->contact_email) }}"
                            class="w-full px-4 py-2 border rounded-lg"></div>
                    <div class="col-span-2"><label class="flex items-center"><input type="checkbox" name="is_active"
                                value="1" {{ $recruitment->is_active ? 'checked' : '' }}
